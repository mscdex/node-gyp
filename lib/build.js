...
function build (gyp, argv, callback) {
  ...
  function doBuild () {
    ...
    if (win) {
      var stdout = '', stderr = '', reErr = /Solution file error MSB5014:/i
      proc.stdout.on('data', function stdoutHandler(data) {
        stdout += data
        if (stdout.length >= 8192) {
          proc.stdout.removeListener(stdoutHandler)
        }
      })
      proc.stderr.on('data', function stderrHandler(data) {
        stderr += data
        if (stderr.length >= 8192) {
          proc.stderr.removeListener(stderrHandler)
        }
      })
    }
    proc.on('exit', function(code, signal) {
      if (win && code !== 0 && (reErr.test(stdout) || reErr.test(stderr))) {
        return callback(new Error('Visual C++/Visual C++ Express 2008+ is required to build this package'))
      }
      onExit(code, signal)
    })
  }
